...
export default function Download({style, packData, id, ...props}: DownloadProps) {
    return <div className="downloadRoot" {...props} style={{animation: 'pullIn 1s', gap: 16, maxWidth: 320}}>
        ...
        {packData.versions.sort((a,b) => -semver.compare(a.name, b.name)).map(v => <PackVersionDownload version={v} packId={id}/>)}
    </div>
}