use std::{collections::HashMap, error::Error, fs, borrow::BorrowMut};

use directories::ProjectDirs;
use mcvm::data::{config::{Config}, user::{User, UserKind, AuthState}};
...
pub fn load_config() -> Result<HashMap<String, User>, Box<dyn Error>> {
    ...
    let config_json_data: String = fs::read_to_string(config_json_path).expect("Failed to read config");

    let config_stored: HashMap<String, UserStored> = serde_json::from_str(&config_json_data).unwrap();
    
    let mut config: HashMap<String, User> = HashMap::new();

    for (user_id, user_data) in config_stored {
        config.insert(user_id, User {
            kind: UserKind::Microsoft,
            id: user_data.id,
            name: user_data.name,
...
pub fn save_config(config: &mut HashMap<String, User>) -> Result<(), Box<dyn Error>> {
    ...
    for (user_id, user_data) in config {
        ...
        match user_data.kind {
            UserKind::Microsoft => {
                config_stored.insert(user_id.to_owned(), UserStored {
...
pub async fn add_user(mc_profile: &MinecraftProfile, access_token: String) -> Result<(), Box<dyn Error>> {
    let mut user_config = load_config()?;
    
    user_config.insert(mc_profile.name.clone(), User { 
        kind: UserKind::Microsoft, 
...
pub async fn switch_user(config: &mut Config, name: String) -> Result<(), Box<dyn Error>> {
    ...
    config.auth.users = users_config;
    config.auth.state = AuthState::Authed(name);

    save_config(&mut config.auth.users)?;
