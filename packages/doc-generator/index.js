...
import { BundleSchema, HTTPResponses, MetaDataSchema, MinecraftVersionSchema, PackCategorySchema, PackDataSchema, PackReferenceSchema, PackVersionSchema, PATokenSchema, SortSchema, UserDataSchema, } from "data-types";
import { Kind, Modifier, } from "@sinclair/typebox";
const defaultVariableTemplate = (groups) => {
    let type = groups[1];
...
const POSSIBLE_DIRECTIVES = {
    ...
    return: {
        pattern: /@return ([^:]+):\s*(.+)/g,
        template: (groups) => `{bdg-primary}\`${groups[0]} - ${HTTPResponses[groups[0]]}\` <label class="sd-text-secondary">${groups[1]}</label>\n`,
    },
    example: {
        pattern: /@example (.+)((\s.*)+)/g,
        template: (groups) => `::::{admonition} ${groups[0]}
    :class: note        
\`\`\`ts${groups[1]}
\`\`\`
::::\n`,
...
function parseDirective(content) {
    for (const name in POSSIBLE_DIRECTIVES) {
        ...
        const match = content
            .matchAll(d.pattern instanceof RegExp
            ? d.pattern
            : new RegExp(d.pattern, "g"))
            .next();
        if (match.done)
            continue;
...
const files = {};
function generateDirectives(type, directives, header) {
    if (directives[type] === undefined || directives[type].length === 0)
        return "";
    return ((header !== undefined ? header + "\n" : "") +
        directives[type]
            .map((groups) => POSSIBLE_DIRECTIVES[type].template(groups))
            .join("\n"));
}
function extract(t, config) {
    ...
    for (let [text, ...other] of blocks) {
        ...
        if (parsedDirectives["route"] === undefined ||
            parsedDirectives["route"].length === 0)
            continue;
        const routeGroup = parsedDirectives["route"][0];
        const route = routeGroup[1];
        const filePath = path.join(config.docRepo, route.replaceAll(":", "") + ".inc");
...
function readDirectory(t, config) {
    if (!fs.existsSync(t)) {
...
function propertyToNiceFormat(key, property, level) {
    ...
    if (registeredSchema !== undefined) {
        ...
        structure += property.anyOf
            .map((p) => propertyToNiceFormat("", p, level))
            .join(" | ");
    }
...
function schemaToNiceFormat(schema, level = 1) {
...
function start(config) {
    ...
    for (const file in files) {
        ...
        file.split(path.sep).forEach((cur, idx, arr) => {
            ...
            if (!fs.existsSync(part) && part !== "")
                fs.mkdirSync(part);
        });
        fs.writeFileSync(file, files[file]);
        imports.push(path
            .join(path.basename(config.docRepo), file.replace(path.normalize(config.docRepo), ""))
            .replaceAll(path.sep, "/"));
    }
    fs.writeFileSync(path.join(config.docRepo, "routes.inc"), imports
        .map((i) => `
\`\`\`{include} ${i}
\`\`\``)
        .join("\n"));
...
if (fs.existsSync("./config.json")) {
    ...
    config.docRepo = path.join(process.env.DOC_REPO ?? config.docRepo, "docs/api/generated");
    start(config);
}
