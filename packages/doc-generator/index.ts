...
const defaultVariableTemplate = (groups: string[]) => {
	...
	if (!["string", "number", "boolean"].includes(typeMatch.value[1])) {
		const schema = registeredSchemas.find(
			(r) => r.name === typeMatch.value[1]
		)
		console.log(schema)
		if (schema !== undefined)
...
const POSSIBLE_DIRECTIVES: { [key: string]: Directive } = {
	...
	return: {
		pattern: /@return ([^:]+):\s*(.+)/g,
		template: (groups) =>
			`{bdg-primary}\`${groups[0]} - ${HTTPResponses[groups[0] as keyof typeof HTTPResponses]}\` <label class="sd-text-secondary">${groups[1]}</label>\n`,
	},
	example: {
		pattern: /@example (.+)((\s.*)+)/g,
		template: (groups) =>
...
function extract(t: string, config: Config) {
	...
	for (let [text, ...other] of blocks) {
		...
		if (
			parsedDirectives["route"] === undefined ||
		...
		else files[filePath] += extractedDoc
	}
}
...
function readDirectory(t: string, config: Config) {
	...
	for (const c of contents) {
		if (c.isDirectory()) readDirectory(path.join(t, c.name), config)
