...
function testSequence(elements: Element[], testTags: string[]) {
	...
	if (tags.length < testTags.length) return false

	for (let i = 0; i < tags.length && i < testTags.length; i++) {
		if (tags[i] !== testTags[i]) return false
	}
	return true
}
...
function featureTemplateTransformer(tree: Node) {
	visit(tree, (node, idx, parent) => {
		...
		if (!testSequence(nonLineBreak, ["h1", "img", "p"])) return

		element.properties = {
			...element.properties,
			className: "markdownFeatureTemplate",
		}

		const headerIdx = element.children.findIndex(
...
export default function MarkdownRenderer({ children, ...props }: any) {
	return (
		<ReactMarkdown
			...
			remarkPlugins={[removeComments, remarkGFM, remarkLink]}
			rehypePlugins={[rehypeRaw, rehypeSanitize, featureTemplate]}
			style={{ ...props.style }}
			{...props}
		>
			{children}
