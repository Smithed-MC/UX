import React, { CSSProperties, MouseEventHandler, createContext, useState } from "react"
...
export default function Modal({ trigger, content, onClose, style, offset }: {
    ...
    return <div className="container" style={{ position: 'relative', ...style }} onMouseLeave={() => setOpen(false)}>
        {trigger}
        <div className="container" style={{ position: 'absolute', top: `calc(100% - ${offset ?? '0.5rem'})`, padding: '1rem 1rem 1rem 1rem', zIndex: open ? 10 : -100, opacity: open ? 1 : 0, transition: 'opacity 0.25s ease-in-out' }}>
