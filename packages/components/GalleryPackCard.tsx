...
export default function GalleryPackCard({ id, packData, packMeta, onClick, state, style, parentStyle, bundleData, user, addWidget, packAuthor, ref, ...props }: PackCardProps) {
    ...
    const [gallery, setGallery] = useState<PackGalleryImage[]|undefined>(undefined)
    ...
    async function populateGallery() {
        const resp = (await fetch(import.meta.env.VITE_API_SERVER + '/packs/' + id + '/gallery'))
            ...
            <div className={`galleryPackCard${displayGallery ? ' displayGallery' : ''}`} key={id} ref={card} onClick={(e) => {
                ...
                <div className='galleryImage' style={{ position: 'relative' }}>
                    ...
                    {gallery && gallery.length > 0 &&
                        <img className='thumbnail' style={{ width: '100%', cursor: 'pointer' }} src={((g: PackGalleryImage) => typeof(g) === 'object' ? g.content ?? `${import.meta.env.VITE_API_SERVER}/packs/${id}/gallery/${currentImage}`  : g)(gallery[currentImage])}
                            onClick={() => { setDisplayGallery(!displayGallery) }} />
                    }
                    {displayGallery && gallery && <div className='carousel'>
                        <button className='buttonLike'
                            style={{ height: '2.5rem', backgroundColor: 'var(--background)' }}
                            onClick={() => {
                                if (currentImage == 0) {
                                    setCurrentImage((gallery.length ?? 1) - 1);
                                } else {
                        ...
                        <div className='container' style={{ padding: '1rem 2rem', backgroundColor: 'var(--background)', borderRadius: '2rem', gap: '1rem', flexDirection: 'row' }}>
                            {gallery.map((_, i) => <CarouselDot key={"carouselDot" + i} selected={currentImage === i} onClick={() => setCurrentImage(i)} />)}
                        </div>
                        <button className='buttonLike'
                            style={{ height: '2.5rem', backgroundColor: 'var(--background)' }}
                            onClick={() => {
                                if (currentImage == (gallery?.length ?? 1) - 1) {
                                    setCurrentImage(0);
                                } else {
                                    setCurrentImage(Math.min(currentImage + 1, (gallery?.length ?? 1) - 1))
                                }
                            }}
                        >
